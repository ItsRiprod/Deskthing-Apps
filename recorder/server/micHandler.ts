...
import { RECORDER_SETTING_IDS } from "../shared/index";
...
const GAP_THRESHOLD = 5000; // 2 seconds gap threshold to finalize recording
...
const saveAudioFile = async () => {
  ...
  const sampleRate: number = Number(
    settings?.[RECORDER_SETTING_IDS.SAMPLE_RATE] || 16000
  );
  const channels: number = Number(
    settings?.[RECORDER_SETTING_IDS.CHANNELS] || 1
  );
  const bitsPerSample: number = Number(
    settings?.[RECORDER_SETTING_IDS.BITS_PER_SAMPLE] || 16
  );
  ...
  // assert with the dirname because the setting received might be a full path to a file (we dont like those people)
  const saveLocation = path.dirname(
    settings?.[RECORDER_SETTING_IDS.SAVE_LOCATION] ||
  ...
  try {
    ...
    const recordingLength =
      audioChunks.length *
      (settings?.[RECORDER_SETTING_IDS.CHUNK_DURATION] || 5);

    recordingsStore.addRecording({
      id: currentRecordingID || filename,
      filename: filename,
      directory: recordingsDir,
      durationSeconds: recordingLength,
...
export const handleBinary = (
  ...
  if (!isRecording) {
    ...
    recordingsStore.setRecording(currentRecordingID, true);
  ...
  console.log(
    `Average volume level: ${roundedLevel} | Recording: ${audioChunks.length} chunks`
  );
};
