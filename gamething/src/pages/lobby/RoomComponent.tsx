  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center mb-2">
        <div
          className="w-6 h-6 sm:w-8 sm:h-8 mr-2 sm:mr-3 rounded-md transition-colors duration-500 flex-shrink-0"
        ...
        {room.status === "waiting" ? (
          ...
          <button
            onClick={onLeaveRoom}
            className="bg-red-600 mr-2 sm:mr-3 hover:bg-blue-700 px-3 sm:px-4 py-1.5 sm:py-2 rounded text-sm"
          >
            {"Exit Room"}
          </button>
        ))}
        <div className="flex-grow min-w-[200px] mt-2 sm:mt-0">
          ...
          <div className="flex items-center mt-1">
            <span
              className={`px-2 py-0.5 text-xs transition-colors duration-500 sm:text-sm flex items-center justify-center rounded mr-2 ${
                room.status === "waiting"
                  ? "bg-yellow-600"
                  : room.status === "playing"
                  ? "bg-green-600"
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            {room.players.map((player) => (
              <div key={player.id} className="flex items-center">
                <div
                  className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full mr-1 transition-colors duration-500"
