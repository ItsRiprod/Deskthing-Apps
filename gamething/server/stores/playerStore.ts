...
class PlayerStore {
  ...
  constructor() {
    DeskThing.getData().then((data) => {
      const players = data?.players as Player[] | undefined;
      if (players) {
        console.log(`Got ${players.length} players from server`)
  ...
  incrementPlayerLosses(id: string) {
    const player = this.players.get(id);
    if (!player) return;
    this.players.set(id, { ...player, losses: (player.losses || 0) + 1 });
    this.notifyPlayerUpdate(id);
  }
}
