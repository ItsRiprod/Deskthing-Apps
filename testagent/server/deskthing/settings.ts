...
export const initSettings = async (): Promise<AppSettings> => {
  ...
  const settings: AppSettings = {
    model: {
      ...
      label: "LLM Model",
      description:
        "WARNING: Research models. Some take more storage and RAM than others. Choose wiseley! Find available models at https://ollama.com/library/",
      value: "gemma3:1b",
      options: [
    ...
    contextLength: {
      ...
      description:
        "How far back in the conversation history the assistant should consider when generating responses. Default is 10.",
      value: "10",
    },
    speakResponses: {
      version: "0.11.0",
    ...
    prompt: {
      ...
      label: "Prompt",
      description:
        "The prompt to use for the assistant. This will be appended to every message.",
      value:
...
const handleNewModel = async (
  newModel: SettingsType,
  model: SettingsType,
  currentModel: string
) => {
  if (
    !newModel ||
    newModel.type !== SETTING_TYPES.STRING ||
    !model ||
    model.type !== SETTING_TYPES.SELECT
  ) {
  ...
  if (!newModelValue || newModelValue.length == 0) {
    console.log(
      "No new model specified or it is the same as the current model."
    );
    return;
  }
  ...
  const existingOptions = (model?.options || []).map((o) =>
    o.value.toLowerCase()
  );
  ...
  const availableLLMS = await getOllamaModels();
  const isInOllamaList = availableLLMS
    .map((m) => m.toLowerCase())
    .includes(newModelValue.toLowerCase());
...
DeskThing.on(DESKTHING_EVENTS.SETTINGS, async (settingsData) => {
  ...
  if (
    !isNaN(contextLength) &&
    contextLength > 0 &&
    contextLength !== existingConfig.maxHistoryLength
  ) {
    console.log(
      `Updating context length from ${existingConfig.maxHistoryLength} to ${contextLength}`
    );
  ...
  if (
    typeof speakResponses === "boolean" &&
    speakResponses !== existingConfig.speakResponses
  ) {
    console.log(
      `Updating speak responses from ${existingConfig.speakResponses} to ${speakResponses}`
    );
