...
import { install, uninstall, InstallConfig, InstallLogger } from '@deskthing/microphone/utils'
...
class ADBPlugin implements ADBPluginInterface {
    ...
    private logger: InstallLogger = (message, error, code) => {
    ...
    public async install(details: ADBPluginDetails): Promise<void> {
        ...
        try {
            ...
            await install(config, this.logger)
            
        } catch (error) {
            console.error(`Error during ADB Plugin installation for ${details.adbId}:`, error);
            throw error
        }
    ...
    public async uninstall(details: ADBPluginDetails): Promise<void> {
        ...
        try {
            await uninstall(config, this.logger)
        } catch (error) {
            console.error(`Error during ADB Plugin uninstallation for ${details.adbId}:`, error);
            throw error
        }
