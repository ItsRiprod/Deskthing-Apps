        ...
        <div className="md:w-1/3">
          <div className="bg-gray-800 flex flex-col p-2 sm:p-4 min-h-fit rounded h-full">
            <div className="flex-shrink-0">
              <h2 className="text-lg sm:text-xl mb-2 sm:mb-4">Player Profile</h2>
              <div className="mb-2 sm:mb-4">
                <div className="flex items-center mb-2 sm:mb-4">
                  ...
                  />
                  <span className="text-xs sm:text-sm ml-2">{currentPlayer?.id}</span>
                </div>
                <p className="text-xs sm:text-sm mb-1 sm:mb-2">Wins: {currentPlayer?.wins || 0}</p>
            ...
            <div className="mt-2 sm:mt-4 min-h-24 flex-grow overflow-y-auto bg-gray-900 p-2 sm:p-4 rounded">
              ...
              {lobby?.players && lobby.players.length > 0 ? (
                    ...
                    .map((player) => (
                      ...
                      >
                        <div className="flex items-center">
                          <div
                            className="w-3 sm:w-4 h-3 sm:h-4 rounded-full mr-1 sm:mr-2"
                            style={{ backgroundColor: player.color }}
                          />
                          <span className="text-xs sm:text-sm">{player.id}</span>
                        </div>
                ...
                <p className="text-xs sm:text-sm text-gray-400">No other players online</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {!lobby?.rooms || lobby.rooms.length === 0 ? (
              <div className="flex flex-col items-center justify-center space-y-2 sm:space-y-4">
                <p className="text-gray-400 text-xs sm:text-sm">No rooms available</p>
              </div>
            ) : (
