...
export class DiscordAuth {
  ...
  async authenticate(): Promise<void> {
    ...
    try {
      ...
      console.log("No valid saved token, starting authorization flow");
      ...
      DeskThing.sendNotification({
        ...
        id: "discord_auth_failed",
        description: `There was an error during Discord authentication. Please check your Client ID, Redirect URL, and Client Secret in the settings and try again. ${
  ...
  private async authorize(): Promise<string> {
    ...
    if (!this.clientId || !this.clientSecret) {
      console.warn(
        "Client ID or Client Secret is not defined - cancelling the authorization"
      );
  ...
  private async authenticateWithToken(token: string): Promise<void> {
    ...
    if (!this.rpc.isConnected) {
      if (!this.clientId) {
        throw new Error(
          "RPC client is not connected and clientId was not provided"
        );
      }
      console.debug(
        "Attempting to reconnect RPC before authenticating with tokens"
      );
